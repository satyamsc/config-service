...
resource "aws_lambda_function" "park_lambda" {
  filename      = "../target/config-service-1.0-SNAPSHOT-lambda-package.zip"
  function_name = "config-lambda"
  role          = aws_iam_role.lambda_role.arn
  handler       = "com.parkhere.configuration.StreamLambdaHandler::handleRequest"
  runtime       = "java17"
  timeout       = 10
}
